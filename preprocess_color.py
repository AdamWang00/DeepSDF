...
def preprocess_color(model_path, texture_path, surface_samples_path, surface_sample_faces_path, sdf_path, ignore_sdf=False, ignore_surface=False):
    # Load SDF samples
    ...
    if ignore_sdf: # if we only want to add color to the surface samples (.ply)
        print("ignoring sdf")
    elif sdf_pos.shape[1] == 7 and sdf_neg.shape[1] == 7:
    ...
    if not ignore_sdf:
        ...
        sdf_pos = np.concatenate((sdf_pos, sdf_pos_color), axis=1)
        sdf_neg = np.concatenate((sdf_neg, sdf_neg_color), axis=1)

        np.savez(sdf_path, pos=sdf_pos, neg=sdf_neg)
...
if __name__ == "__main__":
    ...
    for args in preprocess_color_inputs:
        ...
        preprocess_color(*args, ignore_sdf=ignore_sdf, ignore_surface=ignore_surface)