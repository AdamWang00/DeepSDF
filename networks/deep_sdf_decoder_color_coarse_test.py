...
class Decoder(nn.Module):
    def __init__(
        ...
        # CONV3D_T_DIMS = [latent_size, 512, 512, latent_size]
        # CONV3D_T_KERNEL_SIZES = [2, 1, 1]

        # self.conv3d_t_num_layers = len(CONV3D_T_DIMS)
        # for layer in range(self.conv3d_t_num_layers - 1):
        #     in_channels = CONV3D_T_DIMS[layer]
        #     out_channels = CONV3D_T_DIMS[layer + 1]
        #     print("convT", layer, out_channels)
        #     setattr(self, "convT" + str(layer), nn.ConvTranspose3d(in_channels, out_channels, CONV3D_T_KERNEL_SIZES[layer]))
        ...
        dims = [latent_size // temp_voxel_dim ** 3 + xyz_size] + dims + [4] # TODO: remove // 8
        self.temp_voxel_dim = temp_voxel_dim
    ...
    def forward(self, input, scene_latent=None):
        ...
        latent_vecs = latent_vecs.view(latent_vecs.shape[0], latent_vecs.shape[1] // self.temp_voxel_dim ** 3, self.temp_voxel_dim, self.temp_voxel_dim, self.temp_voxel_dim)
        # for layer in range(self.conv3d_t_num_layers - 1):
        #     convT = getattr(self, "convT" + str(layer))
        #     latent_vecs = convT(latent_vecs)
        #     if (layer < self.conv3d_t_num_layers - 2):
        #         latent_vecs = self.relu(latent_vecs)
    ...
    def forward_alt(self, xyz, latent_vec):
        latent_vec = latent_vec.view(latent_vec.shape[0] // self.temp_voxel_dim ** 3, self.temp_voxel_dim, self.temp_voxel_dim, self.temp_voxel_dim)
        # for layer in range(self.conv3d_t_num_layers - 1):
        #     convT = getattr(self, "convT" + str(layer))
        #     latent_vecs = convT(latent_vecs)
        #     if (layer < self.conv3d_t_num_layers - 2):
        ...
        latent_vec = torch.permute(latent_vec, (1, 2, 3, 0)) # swaps feature (0) with DHW (1, 2, 3)
        xyz_normalized = (xyz.cpu().detach().numpy() + 1) / 2 # should generally be 0 <= x,y,z <= 1
